...
public class Store {
    ...
    public static  void showMessage(boolean isSuccess,String message){

        if (isSuccess){
            new SweetAlertDialog(App.getContext(), SweetAlertDialog.SUCCESS_TYPE)
                    .setTitleText("System alert")
...
//    }
    ...
    public static String getOnlineUserNameById(int id) {
        String userName = null;
        for (int i = 0; i < usersOnline.length(); i++) {
            try {
                if (id == usersOnline.getJSONObject(i).getInt("user_id")) {
                    userName = usersOnline.getJSONObject(i).getString("user_name");
                    break;
                }
            } catch (JSONException e) {
                e.printStackTrace();
